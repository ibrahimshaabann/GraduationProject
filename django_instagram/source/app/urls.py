...
from app.views.user import CreateUser, DestroyUser, LoginUserView, RetrieveUser, UpdateUser, RetrieveCurrentUserProfile
from app.views.post import CommentPost, CreatePost, DestroyPost, FollowUser, RetrievePost  
from app.views.post import RetrieveUserPosts, UpdatePost, LikePost, GetAllPosts , ListNotifications , HandleNotification

urlpatterns = [
    path('user/create/', CreateUser.as_view()),
    path('user/login/', LoginUserView.as_view()),
    path('user/profile/', RetrieveCurrentUserProfile.as_view()),
